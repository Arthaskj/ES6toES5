...
Box.define("HY.rating.warn.negativeOpinions.holdOpinions", {
    ...
    //重要性枚举值--搜索框数据用
    $SearchList: [
        { text: '<font class="fa fa-star" color="#b1b1b1"></font><font class="fa fa-star" color="#b1b1b1"></font><font class="fa fa-star" color="#b1b1b1"></font>', value: "0", title: "0星" },
        { text: '<font class="fa fa-star" color="#ff7800"></font><font class="fa fa-star" color="#b1b1b1"></font><font class="fa fa-star" color="#b1b1b1"></font>', value: "1", title: "1星" },
        { text: '<font class="fa fa-star" color="#ff7800"></font><font class="fa fa-star" color="#ff7800"></font><font class="fa fa-star" color="#b1b1b1"></font>', value: "2", title: "2星" },
        { text: '<font class="fa fa-star" color="#ff7800"></font><font class="fa fa-star" color="#ff7800"></font><font class="fa fa-star" color="#ff7800"></font>', value: "3", title: "3星" },
    ],
    ...
    //渲染表格中特殊数据
    renderTableInTable: function (data, type) {
        var rlt = '<font style="padding:.4em .6em;">--</font>';
        if (!data) return rlt;
        if (data.length > 0) {
            var table = $('<table></table>');
            data.forEach(function (x, index) {
                var tr = $('<tr style="' + (data.length == 1 ? "" : 'background:' + (index % 2 ? "#f5f5f5" : "#fff")) + '" data-bondkey="' + (x.BondKey || "") + '" data-bondname="' + (x.BondName || "") + '" data-compname="' + (x.NewsCompanyName || "") + '" data-compcode="' + (x.NewsCompCode || "") + '">' +
                    '<td style="border-width:0 0 1px 0 !important">' +
                    (type ? (x.BondKey ? '<a class="tdBond" href="javascript:void(0)">' + (x.BondName || "") + "【" + x.BondKey + '】</a>' : (x.BondName || "--")) :
                    (x.NewsCompCode ? '<a class="tdComp" href="javascript:void(0)">' + (x.Relation ? '<font color="#f4333c">【' + x.Relation + '】</font>' : "") + (x.NewsCompanyName + '</a>') : ((x.Relation ? '<font color="#f4333c">【' + x.Relation + '】</font>' : "") + x.NewsCompanyName))) +
                    '</td></tr>')
                table.append(tr);
                tr.data("datas", x);
            })
    ...
    show_window_bond: function (e, target) {
    ...
    show_window_comp: function (e, target) {
        var tr = $(e.target).closest("tr"),
           data = tr.data();
